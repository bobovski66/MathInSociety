...
[243-8,243+8]=[235,251].
\]
Because \(230\) is outside this interval, the sample result is not compatible with the label claim under the interval rule being used in this course.
\end{examplebox}
...
\begin{enumerate}
\item Apple weights are approximately normal with mean \(180\) g and standard deviation \(15\) g.
\begin{enumerate}[label=(\alph*),itemsep=4pt]
\item About what percentage of apples weigh between \(165\) g and \(195\) g?
\item About what percentage weigh more than \(210\) g?
\item Between what two weights do the middle \(95\%\) of apples fall?
\end{enumerate}
\worklarge

\item Student A scores \(520\) on a test with mean \(500\) and standard deviation \(100\). Student B scores \(28\) on a different test with mean \(21\) and standard deviation \(5\).
\begin{enumerate}[label=(\alph*),itemsep=4pt]
...
\item A consumer group tests \(64\) cereal boxes. The sample mean is \(243\) calories per serving, with a reported margin of error of \(\pm8\) calories. The manufacturer's label claims a population mean of \(230\) calories.
\begin{enumerate}[label=(\alph*),itemsep=4pt]
\item Construct the confidence interval.
\item Is the claimed mean of \(230\) inside the interval?
\item In this problem, is \(243\) a population parameter or a sample statistic?
\end{enumerate}
\workxl

\item A study uses a sample of \(n=200\), and its margin of error is approximately proportional to \(1/\sqrt n\). About what sample size would be needed to cut the margin of error in half? Show how the square-root relationship determines your answer.
\worklarge

\item A distribution has mean \(\mu=60\) and standard deviation \(\sigma=12\). An individual value is \(x=78\).
\begin{enumerate}[label=(\alph*),itemsep=4pt]
\item Find the z-score of \(78\).
