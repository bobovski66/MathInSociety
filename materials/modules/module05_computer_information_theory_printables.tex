...
\begin{enumerate}
\item A sensor reports operating mode \(14\). The controller stores the mode in binary.
\begin{enumerate}[label=(\alph*),itemsep=4pt]
\item Convert \(14_{10}\) to binary.
\item Convert the received binary value \(10011_2\) back to base ten.
\end{enumerate}
\worklarge

\item Convert the binary fraction \(0.1101_2\) to a base-ten decimal. Then decide whether each rational number has a finite binary expansion: \(5/16\) and \(2/15\). State the denominator feature you used.
\worklarge

\item A transmitter uses even parity.
\begin{enumerate}[label=(\alph*),itemsep=4pt]
\item Data bits are \(1101\). What parity bit should be appended?
\item A receiver gets the five-bit packet \(11010\). Does the parity check detect an error? Explain by counting the \(1\)s.
\end{enumerate}
\worklarge
...
\item An alarm event has probability \(1/64\). How many bits of information does the event carry according to \(I=-\log_2(p)\)? Show the power-of-two step that makes the calculation immediate.
\workmed
\end{enumerate}
