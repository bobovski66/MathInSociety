...
Throughout this module, use the Chapter 3 standard-deviation convention, which divides by \(n\):
\[
...
\textbf{By the end of this module, you should be able to:} compute mean, median, mode, range, and the Chapter 3 standard deviation; recover a missing value from a known mean; compare consistency using spread; and explain why an outlier can affect the mean more strongly than the median.

\prequizhead{3}{Basic Statistics}
\begin{refbox}{Standard deviation for this module}
\[
...
\item The value \(10\) in Problem 1 came from an unusually difficult day. Remove it and use the remaining data \(3,3,4,5\).
...
\item Five students have a mean test score of \(82\). Four of the scores are \(74,80,86,83\).
\begin{enumerate}[label=(\alph*),itemsep=4pt]
...
\item Using the Chapter 3 convention shown above, compute the standard deviation for the data
\[
4,\ 6,\ 6,\ 8.
\]
...
\item For the data \(2,4,4,6\), the mean is \(4\). Using the Chapter 3 convention, compute the standard deviation by finding and averaging the squared deviations, then taking a square root.
\worklarge
