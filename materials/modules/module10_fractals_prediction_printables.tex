...
\begin{enumerate}
\item Apply the Collatz rule starting from \(9\) until the sequence reaches \(1\).
...
x_{n+1}=2.5x_n(1-x_n)
\]
with \(x_0=0.3\). Compute \(x_1\). Then explain what information would be needed to determine \(x_2\).
\worklarge
...
\end{array}
\]
For each row, compute \(\text{ruler length}\times\text{placements}\). Describe the numerical trend as the ruler gets shorter. Then explain why the trend comes from the measurement scale rather than from a physical change in the coastline.
\workxl
\end{enumerate}
